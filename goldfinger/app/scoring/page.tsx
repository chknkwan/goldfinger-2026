...
export default function ScoringPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-4 pb-10" style={{ background: '#fffbeb' }}>
      {/* Header */}
      ...
      <div className="w-full max-w-md space-y-3">
        {/* Level toggle */}
        <div className="flex bg-white rounded-2xl p-1.5 border-2 border-yellow-200 shadow-sm">
          {(['มต้น', 'มปลาย'] as Level[]).map(lv => (
            <button key={lv} onClick={() => { setLevel(lv); clearForm(); setUserPickedGame(false) }}
              className={`flex-1 py-2.5 rounded-xl font-bold text-sm transition-all ${level === lv ? 'text-white shadow' : 'text-amber-700 hover:text-amber-900'}`}
        ...
        <div className="flex bg-white rounded-2xl p-1.5 border-2 border-yellow-200 shadow-sm">
          {([['qualify', '🎮 รอบคัดเลือก'], ['playoff', '🏆 รอบเพลย์ออฟ']] as [Mode, string][]).map(([m, label]) => (
            <button key={m} onClick={() => { setMode(m as Mode); clearForm() }}
              className={`flex-1 py-2.5 rounded-xl font-bold text-sm transition-all ${mode === m ? 'text-white shadow' : 'text-amber-700'}`}
              style={mode === m ? { background: 'linear-gradient(135deg,#b45309,#f59e0b)' } : {}}>
              {label}
            </button>
          ))}
        </div>

        <form onSubmit={submitResult} className="bg-white rounded-3xl p-5 shadow-sm border border-yellow-200 space-y-4">
          {mode === 'qualify' && (
            <>
              {/* Game pills */}
              <div>
                <label className="block text-xs font-bold text-amber-800 mb-2">🎮 เกมที่</label>
                <div className="flex gap-2 flex-wrap">
                  {Array.from({ length: maxAvailGame || 1 }, (_, i) => i + 1).map(g => (
                    <button key={g} type="button"
                      onClick={() => { setGame(g); setUserPickedGame(true); clearForm() }}
                      className={`relative flex-1 min-w-[44px] py-2.5 rounded-xl font-black text-sm border-2 transition-all active:scale-95 ${game === g ? 'text-white shadow border-amber-700' : 'bg-amber-50 border-yellow-300 text-amber-700 hover:border-amber-400'}`}
                      style={game === g ? { background: 'linear-gradient(135deg,#92400e,#d97706)' } : {}}>
                      {g}
                      {latestGame === g && (
              ...
              {/* Table input */}
              <div>
                <label className="block text-xs font-bold text-amber-800 mb-2">🪑 โต๊ะ</label>
                <div className="flex gap-2 items-stretch">
                  <input
                    ...
                    inputMode="numeric"
                    className="flex-1 px-3 py-3 border-2 border-yellow-200 rounded-xl text-lg font-black text-center bg-amber-50 focus:outline-none focus:border-amber-500"
                  />
                  {(['A', 'B'] as const).map(side => (
                    <button key={side} type="button"
                      onClick={() => setTableSide(prev => prev === side ? '' : side)}
                      className={`w-14 rounded-xl font-black text-xl border-2 transition-all active:scale-95 ${tableSide === side ? 'text-white shadow-md border-amber-700' : 'bg-amber-50 border-yellow-300 text-amber-700 hover:border-amber-500'}`}
                      style={tableSide === side ? { background: 'linear-gradient(135deg,#b45309,#f59e0b)' } : {}}>
                      {side}
                    </button>
                  ))}
                </div>
                ...
                <div className="mt-2 min-h-[1.25rem]">
                  {lookupState === 'loading' && <p className="text-xs font-bold text-amber-500">⏳ กำลังค้นหา...</p>}
            ...
            <>
              <div>
                ...
                <select value={pfRound} onChange={e => { setPfRound(e.target.value); clearForm() }}
                  className="w-full px-3 py-2.5 border-2 border-yellow-200 rounded-xl text-sm font-semibold bg-amber-50 focus:outline-none focus:border-amber-500">
          ...
          {(nameA || nameB) && (
            <div className="flex gap-2 items-center">
              <div className="flex-1 rounded-2xl p-3 border-2 border-yellow-200 bg-amber-50 text-center">
                <p className="text-[10px] font-black text-amber-500 uppercase tracking-wider mb-0.5">ฝั่ง A</p>
                <p className="font-black text-amber-900 text-sm leading-tight">{nameA || '—'}</p>
              </div>
              <div className="font-black text-amber-400 text-xl">VS</div>
              <div className="flex-1 rounded-2xl p-3 border-2 border-yellow-200 bg-amber-50 text-center">
                <p className="text-[10px] font-black text-amber-500 uppercase tracking-wider mb-0.5">ฝั่ง B</p>
                <p className="font-black text-amber-900 text-sm leading-tight">{nameB || '—'}</p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-amber-800 mb-1 text-center truncate">{nameA || 'ผู้เล่น A'}</label>
              <input ref={scoreARef} value={scoreA} onChange={e => setScoreA(e.target.value)}
                placeholder="0" type="number" inputMode="numeric"
                className="w-full px-3 py-4 border-2 border-yellow-200 rounded-xl text-2xl font-black text-center bg-amber-50 focus:outline-none focus:border-amber-500" />
            </div>
            <div>
              <label className="block text-xs font-bold text-amber-800 mb-1 text-center truncate">{nameB || 'ผู้เล่น B'}</label>
          ...
          {confirmOverwrite && (
            <div className="rounded-2xl p-4 border-2 border-amber-300 bg-amber-50 space-y-3">
              <p className="text-sm font-black text-amber-800">⚠️ โต๊ะ {subTable} เกม {game} มีผลอยู่แล้ว</p>
              <p className="text-xs text-amber-700">ผลเดิม: <strong>{existingResult?.score1} – {existingResult?.score2}</strong></p>
              <p className="text-xs text-amber-600">ต้องการเขียนทับด้วย {confirmOverwrite.scoreA} – {confirmOverwrite.scoreB} หรือไม่?</p>
